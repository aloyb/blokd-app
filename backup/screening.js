...
function getRawPoolScreeningRejectReason(pool, s) {
  ...
  if (s.minVolatility != null && volatility < s.minVolatility) return `volatility ${volatility} below minVolatility ${s.minVolatility}`;
...
export async function getTopCandidates({ limit = 10 } = {}) {
  ...
  const minVolatility = config.screening.minVolatility == null ? null : Number(config.screening.minVolatility);
  ...
  const eligible = pools
    .filter((p) => {
      ...
      if (Number.isFinite(minVolatility) && volatility < minVolatility) {
        pushFilteredReason(filteredOut, p, `volatility ${volatility} below minVolatility ${minVolatility}`);
        return false;
      }
