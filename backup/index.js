...
export async function runScreeningCycle({ silent = false } = {}) {
  ...
  try {
    ...
    // If no candidates at all from API, return early without LLM call
    if (candidates.length === 0) {
      ...
      screenReport = `No candidates available right now.\n${thresholds}`;
      appendDecision({
        type: "no_deploy",
        actor: "SCREENER",
        summary: "No candidates from API",
        reason: "getTopCandidates returned empty list",
      });
      return screenReport;
    }
...
IMPORTANT:
- If no candidate qualifies, you MUST call the skip_cycle tool with a brief reason. Do NOT respond with plain text.
...
- Keep the whole report compact and highly scannable for Telegram.
      `, config.llm.maxSteps, [], "SCREENER", config.llm.screeningModel, 2048, {
        ...
        onToolFinish: async ({ name, result, success }) => {
          ...
          if (name === "skip_cycle") {
            screenReport = `⛔ NO DEPLOY\n\nCycle skipped: ${result?.reason ?? "no candidates qualified"}`;
          }
...
async function deployLatestCandidate(index) {
  ...
  const strategy = candidate.volatility <= 5 ? "bid_ask" : "spot";
  const binsBelow = computeBinsBelow(candidate.volatility);
  const binsAbove = 0; // single-sided SOL only
