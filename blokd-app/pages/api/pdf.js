...
export default function handler(req, res) {
  ...
  const colors = {
    ink: '#1F2937', muted: '#6B7280', line: '#D8E0EA', soft: '#EEF6FF',
    softAlt: '#F8FBFF', brand: '#1667B7', brandDark: '#0F4F93',
  ...
  const totalDana = data.members.reduce((sum, m) => sum + totalForMember(m), 0);
  ...
  const totalPeriodeBayar = data.members.reduce((sum, m) => sum + paidMonths(m), 0);
  const belumAdaBayarCount = data.members.filter((m) => paidMonths(m) === 0).length;

  const doc = new PDFDocument({ size: 'A4', layout: 'landscape', margin: 28, autoFirstPage: false });

  res.setHeader('Content-Type', 'application/pdf');
  res.setHeader('Content-Disposition', 'attachment; filename="Laporan-Iuran-BlokD-2026.pdf"');
  doc.pipe(res);
